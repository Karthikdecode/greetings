  ...
  return (
    ...
    >
      ...
      <motion.div
        initial={{ scale: 0.8, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl p-10 text-center max-w-sm z-10"
      >
        <span className="material-symbols-outlined text-8xl text-red-400">
          mail
        </span>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          onClick={() => router.push("/valentine")}
          className="bg-pink-500 text-white px-8 py-3 rounded-full font-bold"
        >
          Open Mail 💖
        </motion.button>
      </motion.div>

      <p className="absolute bottom-6 text-sm text-red-500/50">
        made with love for Prethika 💗
      </p>
    </div>
  );
